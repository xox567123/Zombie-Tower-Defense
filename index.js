...
const humanShop = document.getElementById("humanShop");
const missionsPage = document.getElementById("missionsPage");
...
const evolutionArea = document.getElementById("evolutionArea");
...
const closeTowerButton = document.getElementById("closeTowerButton");
const resultOverlay = document.getElementById("resultOverlay");
const resultTitle = document.getElementById("resultTitle");
const resultText = document.getElementById("resultText");
const continueButton = document.getElementById("continueButton");
const resultLobbyButton = document.getElementById("resultLobbyButton");
const WIDTH = 1200;
const HEIGHT = 600;
const MAX_HEALTH = 150;
const SAVE_KEY = "zombieDefenseV5BossInvasionExpanded";
const EVOLUTION_COST = 1000;
const OLD_SAVE_KEYS = [
"zombieDefenseV4Fixed",
"zombieDefenseAccountSaveV6",
"zombieDefenseAccountSaveV5",
"zombieDefenseAccountSaveV4",
"zombieDefenseAccountSaveV3",
"zombieDefenseAccountSaveV2"
];
const targetingModes = ["first", "last", "strong", "weak"];
...
icon: "🧑",
cost: 0,
description: "Balanced damage, range, and attack speed.",
damage: 38,
range: 170,
rate: 2.5
},
...
icon: "🧑‍⚕️",
cost: 75,
description: "Restores 3 base health after every cleared wave.",
damage: 22,
range: 160,
rate: 2,
heal: 3
...
icon: "👷",
cost: 110,
description: "Boosts nearby tower damage by 15%.",
damage: 27,
range: 165,
rate: 2.2,
auraDamage: 0.15,
...
icon: "🧑‍🚒",
cost: 150,
description: "Burns zombies and deals splash damage.",
damage: 31,
range: 155,
rate: 1.9,
splash: 45,
...
icon: "🪖",
cost: 220,
description: "Boosts nearby attack speed and earns $20 each wave.",
damage: 45,
range: 185,
rate: 2.1,
auraSpeed: 0.15,
...
{ x: 1030, y: 110, r: 55, color: "#5d4037" }
]
},
{
id: "ice",
...
{ x: 770, y: 120, w: 135, h: 170, color: "#263238" },
{ x: 1030, y: 430, w: 145, h: 140, color: "#37474f" }
]
},
{
id: "space",
...
{ x: 820, y: 410, r: 75, color: "#000" },
{ x: 1040, y: 140, r: 65, color: "#000" }
]
},
{
id: "candy",
...
{ x: 800, y: 130, r: 70, color: "#ec407a" },
{ x: 1050, y: 450, r: 65, color: "#fff176" }
]
},
{
id: "jungle",
...
{ x: 735, y: 90, r: 62, color: "#2e7d32" },
{ x: 1010, y: 170, r: 72, color: "#33691e" }
]
},
{
id: "graveyard",
...
{ x: 705, y: 325, w: 70, h: 105, color: "#263238" },
{ x: 1000, y: 470, w: 80, h: 105, color: "#212121" }
]
},
{
id: "ocean",
...
{ x: 780, y: 400, r: 75, color: "#01579b" },
{ x: 1050, y: 125, r: 65, color: "#0288d1" }
]
},
{
id: "moon",
...
{ x: 700, y: 260, r: 70, color: "#111" },
{ x: 1010, y: 185, r: 78, color: "#111" }
]
},
{
id: "factory",
...
{ x: 745, y: 320, w: 120, h: 150, color: "#263238" },
{ x: 1090, y: 100, w: 120, h: 110, color: "#37474f" }
]
},
{
id: "ruins",
...
{ x: 710, y: 260, w: 90, h: 125, color: "#6d4c41" },
{ x: 1030, y: 120, w: 110, h: 110, color: "#795548" }
]
},
{
id: "storm",
...
{ x: 760, y: 425, r: 70, color: "#37474f" },
{ x: 1050, y: 140, r: 68, color: "#455a64" }
]
},
{
id: "abyss",
...
{ x: 720, y: 190, r: 75, color: "#000" },
{ x: 1050, y: 470, r: 72, color: "#000" }
]
}
];
const missions = {
...
permanentCashLevel: 0,
completedMaps: {},
ownedHumans: ["survivor"],
equippedHuman: "survivor",
totalKills: 0,
completedMissions: {}
};
let currentMap = null;
...
let gameTime = 0;
let lastTimestamp = 0;
let bossStartHealth = MAX_HEALTH;
let overdriveUntil = 0;
let abilityReady = {};
let runUsedFarm = false;
let runOnlyHumans = true;
let runPlacedTower = false;
...
freeTowerType:
data.freeTowerType &&
towerTypes[data.freeTowerType] &&
data.freeTowerType !== "waste"
? data.freeTowerType
: null,
totalRound10Wins: Math.max(0, Number(data.totalRound10Wins) || 0),
unlockedMapCount: Math.max(
1,
Math.min(maps.length, Number(data.unlockedMapCount) || 1)
),
cashBoosts: Math.max(0, Number(data.cashBoosts) || 0),
permanentDamageLevel: Math.max(
0,
Math.min(10, Number(data.permanentDamageLevel) || 0)
),
permanentCashLevel: Math.max(
0,
Math.min(10, Number(data.permanentCashLevel) || 0)
),
completedMaps:
data.completedMaps && typeof data.completedMaps === "object"
? data.completedMaps
: {},
ownedHumans: owned,
equippedHuman: owned.includes(data.equippedHuman)
? data.equippedHuman
: "survivor",
totalKills: Math.max(0, Number(data.totalKills) || 0),
completedMissions:
data.completedMissions && typeof data.completedMissions === "object"
...
if (saved) {
try {
account = cleanAccount(JSON.parse(saved));
} catch (error) {
...
const data = {
game: "Zombie Defense",
version: "V5 Boss Invasion Expanded",
...account
};
...
link.href = url;
link.download = "zombie-defense-v5-expanded-save.json";
link.click();
...
reader.onload = () => {
try {
account = cleanAccount(JSON.parse(reader.result));
saveAccount(false);
renderHumans();
renderMissions();
setMessage("Save file loaded.");
} catch (error) {
setMessage("That save file did not work.");
}
event.target.value = "";
...
function permanentDamageMultiplier() {
return 1 + account.permanentDamageLevel * 0.05;
}
function permanentStartingCash() {
return account.permanentCashLevel * 50;
}
function permanentDamageCost() {
return 75 + account.permanentDamageLevel * 50;
}
function permanentCashCost() {
...
: "None";
document.getElementById("cashBoostText").textContent = account.cashBoosts;
document.getElementById("permDamageText").textContent =
account.permanentDamageLevel * 5;
...
const boostButton = document.getElementById("cashBoostButton");
const damageMetaButton = document.getElementById("permDamageButton");
const cashMetaButton = document.getElementById("permCashButton");
rollButton.disabled =
account.zombieBucks < 100 || Boolean(account.freeTowerType);
boostButton.disabled = account.zombieBucks < 50;
if (account.permanentDamageLevel >= 10) {
damageMetaButton.textContent = "Permanent Damage: MAXED";
damageMetaButton.disabled = true;
} else {
...
.map((map, index) => {
const unlocked = index < account.unlockedMapCount;
const completed = Boolean(account.completedMaps[map.id]);
const defaultDifficulty = recommendedDifficulty(map);
  return `
    <div class="mapCard ${unlocked ? "" : "locked"} ${
      completed ? "completed" : ""
    }">
      <h2>${map.name}</h2>
      <p>${map.description}</p>
...
.map(([id, human]) => {
const owned = account.ownedHumans.includes(id);
const equipped = account.equippedHuman === id;
  return `
    <div class="humanCard ${equipped ? "equipped" : ""}">
      <div class="humanIcon">${human.icon}</div>
      <h2>${human.name}</h2>
...
.map(([id, mission]) => {
const completed = Boolean(account.completedMissions[id]);
  return `
...
account.cashBoosts++;
saveAccount(false);
setMessage("Your next run will receive an extra $150.");
}
function buyPermanentDamage() {
const cost = permanentDamageCost();
if (
account.permanentDamageLevel >= 10 ||
account.zombieBucks < cost
) {
return;
}
account.zombieBucks -= cost;
account.permanentDamageLevel++;
saveAccount(false);
setMessage(
"Permanent damage increased to +" +
account.permanentDamageLevel * 5 +
"%."
);
}
function buyPermanentCash() {
const cost = permanentCashCost();
if (
account.permanentCashLevel >= 10 ||
account.zombieBucks < cost
) {
return;
}
account.zombieBucks -= cost;
account.permanentCashLevel++;
saveAccount(false);
setMessage(
"Permanent starting cash increased to +$" +
permanentStartingCash() +
"."
);
}
function buildGameButtons() {
towerBar.innerHTML = Object.entries(towerTypes)
.map(
...
bossStartHealth = MAX_HEALTH;
overdriveUntil = 0;
abilityReady = {};
runUsedFarm = false;
runOnlyHumans = true;
runPlacedTower = false;
lobby.classList.remove("visible");
humanShop.classList.remove("visible");
missionsPage.classList.remove("visible");
resultOverlay.classList.remove("visible");
towerBar.classList.remove("hidden");
...
const first = currentMap.path[i];
const second = currentMap.path[i + 1];
if (
...
for (const object of currentMap.obstacles) {
if (object.r) {
if (
...
x > object.x - object.w / 2 - 27 &&
x < object.x + object.w / 2 + 27 &&
y > object.y - object.h / 2 - 27 &&
y < object.y + object.h / 2 + 27
) {
return false;
}
...
return !towers.some(
(tower) => Math.hypot(x - tower.x, y - tower.y) < 50
);
}
...
let humanType = null;
if (type === "human") {
humanType = account.equippedHuman;
stats = {
  ...base,
...
name:
type === "human"
? humanTypes[humanType].name + " Human"
: base.name,
icon:
type === "human"
? humanTypes[humanType].icon
: base.icon,
color: base.color,
x,
y,
cost: base.cost,
spent: base.cost,
damage: (stats.damage || 0) * permanentDamageMultiplier(),
range: stats.range || 0,
rate: stats.rate || 0,
...
heal: stats.heal || 0,
auraDamage: stats.auraDamage || 0,
auraSpeed: stats.auraSpeed || 0,
auraRange: stats.auraRange || 0,
detectInvisible: Boolean(stats.detectInvisible),
nonAttacking: Boolean(stats.nonAttacking),
targeting: "first",
upgrades: {
damage: 0,
range: 0,
...
const ratio = distance / segments[i];
  return {
    x:
      points[i][0] +
      (points[i + 1][0] - points[i][0]) * ratio,
    y:
      points[i][1] +
      (points[i + 1][1] - points[i][1]) * ratio
  };
}
...
let color = "#43a047";
let reward = 10;
let leakDamage = 5 + Math.floor(wave / 2);
if (type === "fast") {
speed *= 1.55;
...
if (boss) {
healthMultiplier = 18 + wave * 0.7;
speed *= 0.5;
radius = 36;
color = currentMap.bossColor;
reward = 150 + wave * 5;
leakDamage = 45;
}
...
enemies.push(
createEnemy(chooseEnemyType(i), -0.08 * i, false)
);
}
if (wave % 10 === 0) {
...
function towerDamageMultiplier(tower) {
let multiplier = 1;
...
function towerSpeedMultiplier(tower) {
let multiplier = gameTime < overdriveUntil ? 2 : 1;
for (const other of towers) {
if (
other !== tower &&
other.auraSpeed > 0 &&
...
money += enemy.reward;
account.totalKills++;
if (account.totalKills >= 1000) {
completeMission("thousandKills");
}
if (enemy.type === "splitter" && !enemy.child) {
for (let i = 0; i < 2; i++) {
...
towers.forEach((tower) => {
  account.zombieBucks += tower.bossBuck || 0;
});

if (health >= bossStartHealth) {
  completeMission("perfectBoss");
}

saveAccount(false);
showWarning(currentMap.boss + " defeated!");
...
target.freezeUntil > gameTime)
) {
damage *= tower.frozenBonus;
}
if (
tower.critChance > 0 &&
...
return;
  ...
  if (
    ...
    damageEnemy(other, damage * 0.5, tower);
  }
});
...
for (let jump = 0; jump < tower.chain; jump++) {
  ...
  hit.add(next.enemy);
  damageEnemy(next.enemy, damage * 0.62, tower);
  current = next.enemy;
}
...
target.progress - enemy.progress < 0.17
)
.sort((a, b) => b.progress - a.progress)
.slice(0, tower.pierce - 1)
.forEach((enemy, index) => {
damageEnemy(
enemy,
damage * Math.max(0.35, 0.75 - index * 0.06),
tower
);
});
}
if (tower.slow < 1) {
target.slowUntil = gameTime + 2.5;
target.slowAmount = tower.slow;
}
if (tower.poison > 0) {
target.poisonUntil = gameTime + 4;
...
: 1.8;
towers.forEach((tower) => {
  tower.stunnedUntil = gameTime + duration;
});

showWarning(
  currentMap.boss.toUpperCase() +
    " USED " +
    currentMap.bossPower.toUpperCase() +
    "!"
);
...
if (enemy.poisonUntil > gameTime) {
  enemy.poisonTimer += delta;

  if (enemy.poisonTimer >= 0.5) {
    enemy.poisonTimer = 0;

    damageEnemy(enemy, enemy.poisonDamage, {
      ignoreArmor: true
    });
  }
}

if (enemy.burnUntil > gameTime) {
  enemy.burnTimer += delta;

  if (enemy.burnTimer >= 0.45) {
    enemy.burnTimer = 0;
    damageEnemy(enemy, enemy.burnDamage, {});
  }
}
...
if (enemy.type === "healer") {
  ...
  if (enemy.healTimer >= 1) {
    ...
    enemies.forEach((other) => {
      ...
      if (
        Math.hypot(
          position.x - healerPosition.x,
          position.y - healerPosition.y
        ) <= 130
      ) {
...
if (enemy.freezeUntil > gameTime) {
  speed = 0;
} else if (enemy.slowUntil > gameTime) {
...
let healing = 0;
towers.forEach((tower) => {
income += tower.income || 0;
healing += tower.heal || 0;
});
money += income;
health = Math.min(MAX_HEALTH, health + healing);
...
const settings = difficultyData();
account.zombieBucks += settings.bucks;
account.totalRound10Wins++;
account.completedMaps[currentMap.id] = true;
const index = maps.findIndex(
(map) => map.id === currentMap.id
);
let unlockText = "";
if (
index === account.unlockedMapCount - 1 &&
account.unlockedMapCount < maps.length
) {
account.unlockedMapCount++;
unlockText =
  " You unlocked " +
  maps[account.unlockedMapCount - 1].name +
  "!";
...
completeMission("hardVictory");
}
saveAccount(false);
paused = true;
resultTitle.textContent = "Map Complete!";
...
function totalUpgradeCount(tower) {
return (
...
function damageUpgradeCost(tower) {
return 100 + tower.upgrades.damage * 65;
}
function rangeUpgradeCost(tower) {
return 120 + tower.upgrades.range * 70;
}
function speedUpgradeCost(tower) {
      ...
      (evolution, index) => `
        <div class="evolutionCard">
          ...
          <button
            ...
            ${money < EVOLUTION_COST ? "disabled" : ""}
          >
            Evolve into ${evolution.name} — ${EVOLUTION_COST}
          </button>
        </div>
      `
    )
...
evolutionArea
  .querySelectorAll("button[data-evolution]")
  .forEach((button) => {
    button.addEventListener("click", () => {
      evolveTower(
        tower,
        Number(button.dataset.evolution)
      );
    });
  });
...
evolutionArea.innerHTML = `
<h3>Tower Evolution</h3>
  <div class="panelInfo">
    Purchase five total upgrades to unlock evolution.<br>
    Progress: ${totalUpgradeCount(tower)} / 5
  </div>
`;
...
tower.upgrades.damage++;
if (isIncomeTower(tower)) {
tower.income += tower.type === "farm" ? 45 : 25;
setMessage(
  tower.name +
    " income upgraded to $" +
    Math.round(tower.income) +
    " per wave."
);
...
tower.poison *= 1.15;
tower.burn *= 1.15;
setMessage(tower.name + " damage upgraded.");
...
tower.upgrades.range++;
tower.range += 25;
setMessage(tower.name + " range upgraded.");
showTowerPanel(tower);
updateUI();
}
...
tower.upgrades.speed++;
tower.rate *= 1.2;
setMessage(tower.name + " attack speed upgraded.");
showTowerPanel(tower);
updateUI();
}
function evolveTower(tower, index) {
if (
tower.evolution ||
totalUpgradeCount(tower) < 5 ||
...
showWarning("TOWER EVOLVED: " + evolution.name);
showTowerPanel(tower);
updateUI();
}
...
damageEnemy(
enemy,
Math.max(250, enemy.maxHealth * 0.3),
{ ignoreArmor: true }
);
}
});
setMessage("Air Strike launched!");
...
if (id === "supplies") {
money += 500;
setMessage("Emergency Supplies delivered $500.");
}
if (id === "repair") {
health = Math.min(MAX_HEALTH, health + 40);
setMessage("Base Repair restored 40 health.");
}
if (id === "meteor") {
enemies.forEach((enemy) => {
if (!enemy.dead && enemy.progress >= 0) {
...
setMessage("Meteor strike!");

}
abilityReady[id] = gameTime + ability.cooldown;
updateUI();
}
...
const id = button.dataset.ability;
  const unlocked = towers.some(
    (tower) => tower.ability === id
  );
...
.find(
(item) =>
Math.hypot(item.x - x, item.y - y) <= 25
);
if (tower) {
showTowerPanel(tower);
} else {
...
setMessage(
"That position is blocked by the path, terrain, or another tower."
);
return;
}
placeTower(
...
setMessage("Tower sold for $" + refund + ".");
hideTowerPanel();
updateUI();
});
...
document
.getElementById("humanShopButton")
.addEventListener("click", () => {
renderHumans();
humanShop.classList.add("visible");
});
document
.getElementById("closeHumanShop")
.addEventListener("click", () => {
humanShop.classList.remove("visible");
renderLobby();
});
...
.addEventListener("click", () => {
renderMissions();
missionsPage.classList.add("visible");
});
document
...
document
.getElementById("rollTowerButton")
.addEventListener("click", rollFreeTower);
document
.getElementById("cashBoostButton")
...
.addEventListener("change", uploadSave);
function drawBackground() {
ctx.fillStyle = currentMap
...
if (currentMap && currentMap.id === "storm") {
ctx.strokeStyle = "#fff17688";
ctx.lineWidth = 3;
for (let i = 0; i < 8; i++) {
  const x =
    (i * 181 + gameTime * 25) % WIDTH;
...
ctx.lineWidth = 94;
ctx.stroke();
ctx.strokeStyle = currentMap.road;
ctx.lineWidth = 82;
ctx.stroke();
ctx.setLineDash([18, 18]);
ctx.strokeStyle = currentMap.line;
ctx.lineWidth = 8;
ctx.stroke();
ctx.setLineDash([]);
}
...
ctx.strokeStyle = tower.evolution
  ? "#ffeb3b"
  : "#0008";

ctx.lineWidth = tower.evolution ? 5 : 3;
ctx.stroke();
...
if (tower.stunnedUntil > gameTime) {
  ctx.font = "18px Arial";
  ctx.fillText(
    "💫",
    tower.x + 18,
...
effects = effects.filter(
  (effect) => effect.expires > gameTime
);
